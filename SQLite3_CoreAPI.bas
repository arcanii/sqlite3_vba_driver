Attribute VB_Name = "SQLite3_CoreAPI"
'==============================================================================
' SQLite3_CoreAPI.bas  -  Core API: DLL loader, wrappers, and helpers
'
' This single module consolidates what was formerly three separate files:
'   SQLite3_API.bas     - LoadLibrary / DispCallFunc loader + all SQLite wrappers
'   SQLite3_API_Ext.bas - Auxiliary dispatch bridge (secondary handle copy)
'   SQLite3_Helpers.bas - Public utility helpers (QueryScalar, TableExists, etc.)
'
' Combining them reduces the minimum installation to:
'   SQLite3_CoreAPI.bas   <- this file (required)
'   SQLite3Connection.cls (required)
'   SQLite3Recordset.cls  (required)
'   SQLite3Fields.cls     (required)
'   SQLite3Field.cls      (required)
'   SQLite3Command.cls    (required)
'   ...plus any optional feature .bas / .cls files you want
'
...
'            UTF-8 marshalling, all core SQLite3 wrappers. Helpers: QueryScalar,
'            TableExists, TableRowCount, RecordsetToRange, BindParamIndex.
...
'            Helpers: added GetQueryPlan.
'   0.1.6 - Helpers: added QueryColumn.
'            Added SQLITE_OPEN_READONLY and SQLITE_CHECKPOINT_* constants.
'   0.1.7 - Consolidated SQLite3_API.bas + SQLite3_API_Ext.bas +
'            SQLite3_Helpers.bas into this single SQLite3_CoreAPI.bas.
'            No functional changes; all public symbols unchanged.
'
'
...
Private m_pBindParamIdx As LongPtr    ' proc address for bind_parameter_index
...
Public Function SQLite_Load(ByVal dllPath As String) As Boolean
    ...
    If m_hDll = 0 Then
        Err.Raise vbObjectError + 1, "SQLite3_CoreAPI.SQLite_Load", _
...
'==============================================================================
' Auxiliary dispatch bridge
' DispatchProc and BindParamIdxProc were formerly in SQLite3_API_Ext.bas.
' SetDllHandle logic is now inlined into SQLite_Load above.
'==============================================================================

' General-purpose dispatcher for auxiliary procs.
...
'==============================================================================
' Helpers  (formerly SQLite3_Helpers.bas)
' High-level utilities built on top of the core wrappers above.
'==============================================================================
